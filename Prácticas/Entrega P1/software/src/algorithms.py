...
def classification_rate(w,features,targets):
    n = features.shape[0]   # amount of rows
    rights = 0
    loo = LeaveOneOut()
    for train_index,test_index in loo.split(features):
        if weighted_onenn(w, features[train_index], targets, features[test_index]) == targets[test_index]:
            rights += 1
...
"""Local Search Algorithm.
    ...
    """
def localSearch(ini_weight,features,targets):
    n = features.shape[1]
    count = 0
    improve = False
