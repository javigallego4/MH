...
num\_successes != 0 and $T_{current} > T_F$ and iters $\geq$ max\_iterations
...
\State weights $\gets$ generamos la solución inicial
\State weights $\gets$ aplicamos búsqueda local para 15000 / 15 evaluaciones máximas
\State bestF $\gets$ f(weights)
...
\Procedure{ILS}{data,classes, trainIndex, testIndex}
\State weights $\gets$ generamos la solución inicial
\State weights $\gets$ aplicamos enfriamiento simulado para 15000 / 15 evaluaciones máximas
\State bestF $\gets$ f(weights)
\State best\_weigths $\gets$ weights.copy()
\State mutations $\gets$ range(0.1 * nº features)
\State
...
\State \hspace{1cm}weigths $\gets$ mute(weights, index, 0.4)
\State \hspace{0.5cm}\emph{endloop}
\State \hspace{0.5cm}weights $\gets$ aplicamos enfriamiento simulado para 15000 / 15 evaluaciones máximas
\State \hspace{0.5cm}currentF $\gets$ f(weights)
